...
const fetchCountries = (txt)=>{
    ...
    fetch((0, _constantsJs.COUNTRIES_API)(txt)).then((response)=>{
        if (!response.ok) {
        ...
        if (countries.length > 1 && countries.length < 11) {
            const countriesArray = countries.map((item)=>`<li><img style="width: 30px" src=${item.flags.svg} />${item.name.common}</li>`).join("");
            countryList.innerHTML = countriesArray;
        } else if (countries.length === 1) countryList.innerHTML = `<h2>
      ...
      `;
        ...
        } else if (countries.length === 0) countryList.innerHTML = `<h2>Нет результатов</h2>`;
    }).catch((err)=>console.log("error"));
};
const fetchCountriesDebounced = (0, _lodashDebounceDefault.default)(fetchCountries, (0, _constantsJs.DEBOUNCE_DELAY));
const searchBox = document.getElementById("search-box");
searchBox.addEventListener("input", function(e) {
    fetchCountriesDebounced(e.target.value);
});

},{"./css/styles.css":"1CY4s","lodash.debounce":"3JP5n","notiflix":"5z0Oc","./constants.js":"3huJa","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"1CY4s":[function() {},{}],"3JP5n":[function(require,module,exports) {
/**
...
parcelHelpers.export(exports, "DEBOUNCE_DELAY", ()=>DEBOUNCE_DELAY);
parcelHelpers.export(exports, "COUNTRIES_API", ()=>COUNTRIES_API);
